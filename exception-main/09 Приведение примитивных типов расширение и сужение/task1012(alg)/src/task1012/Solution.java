...
public class Solution {
    public static void main(String[] args) throws Exception {
        ...
        LinkedList<Character> listSymbol = new LinkedList<>();
        for (String x : list) {
            String line = x.toLowerCase();
            for (int i = 0; i < line.length(); i++) {
                listSymbol.add(line.charAt(i));
            }
        }

        for (Character ab: alphabet){
            int counter = 0;
            for (Character ls: listSymbol){
                if (ab.equals(ls)){
                    counter++;
                }
            }
